    ...
    if config.rnaseq.aligner == "star_salmon":
        samples.output |= {
            OutputGlob(
                ...
                dst_dir="{sample.id}_{sample.last_run}_{timestamp}/expression/salmon/",
                dst_name=None,
            ),
            OutputGlob(
    ...
    if config.rnaseq.aligner == "star_rsem":
        samples.output |= {
            OutputGlob(
                ...
                dst_dir="{sample.id}_{sample.last_run}_{timestamp}/expression/rsem/",
                dst_name=None,
            ),
            OutputGlob(
...
@output(
    "salmon",
    dst_name="{sample.id}_{sample.last_run}_{timestamp}/expression/salmon_pseudo",
)
@output(
...
@output(
    ...
    dst_dir="{sample.id}_{sample.last_run}_{timestamp}/expression/qc/",
)
@output(
...
@output(
    ...
    dst_name="{sample.id}_{sample.last_run}_{timestamp}/multiqc",
)
@output(
    "pipeline_info",
    dst_name="{sample.id}_{sample.last_run}_{timestamp}/pipeline_info/rnaseq",
)
@runner(split_by="id")
def rnaseq(
    samples: Samples,
